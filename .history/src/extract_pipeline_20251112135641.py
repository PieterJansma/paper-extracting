...
def _with_messages(text_slice: str) -> List[Dict[str, str]]:
    user_msg = build_prompt_n_included_and_countries(text_slice)
    return [
        {"role": "system", "content": (
...
def _try_chat(client: OpenAICompatibleClient, messages: List[Dict[str, str]], temperature: float, max_tokens: int, grammar):
    return client.chat(
        messages,
...
def extract_n_included_and_countries(
    ...
    *,
    ...
    messages = _with_messages(paper_text[:200000])
    ...
    if data is None or (data.get("n_included") is None and not data.get("countries")):
        ...
        for part in _chunk_text(paper_text, max_chars=40000):
            try:
                raw = _try_chat(
                    client, _with_messages(part), temperature, max_tokens,
                    GRAMMAR_JSON_INT_OR_NULL if use_grammar else None
                )
                d = _json_safe_load(raw)
